...
export function resultPageScript(result: DetectionResult) {
  const classes = result.players.map((player) => cssCardClass(player));
  return `
${mobileCheck()}
function getScale() {
  const legend = document.getElementById("legend");
  const backBtn = document.getElementById("back-btn");
  const usedHeight = legend.offsetHeight + backBtn.offsetHeight + 12;
  return Math.min((window.innerWidth - 50) / ${result.textBox.width}, (window.innerHeight - usedHeight) / ${result.textBox.height});
}
function addOnMouseOverBehaviour(className) {
  ...
  function activate() {
    for (let j = 0; j < elements.length; j++) {
      ...
      // since we are scaling the img-wrapper, we need to unscale this width
      elements[j].style.strokeWidth = "calc(4px / " + getScale() + ")";
    }
  }
  function deactivate() {
    for (let j = 0; j < elements.length; j++) {
...
function scaleResult() {
  ...
  for (let i = 0; i < elementsToScale.length; i++) {
    elementsToScale[i].style.transform = "scale(" + scale + ")";
  }
}
